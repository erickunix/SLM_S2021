...
ls() #environment contents
rm(number) #removing objects
rm(number2,next_number,number3) #removing multiple objects

#### Vectors ####

my_first_vector = c(4,7,23.5,76.2,80)
my_first_vector
length(my_first_vector) #vector's length
mode(my_first_vector) #type of vector's contents
...
set.seed(1) #fixing randomness
los=rnorm(1000) #1000 random samples
hist(los,freq=0) #histogram
lines(density(los),col="blue") #density estimation
...
array3D=array(1:24,dim=c(4,3,2)) #tensor
array3D
array3D[1,3,2]
array3D[1,,2]
array3D[4,3,]
...
matrix1*matrix2
t(matrix2) #transpose
matrix1%*%t(matrix2) #multiplication
